...
class ROB extends Module with CPUConfig {
  ...
  when(io.controlInit.fire && io.dataInit.fire) {
    ...
    // 更新队尾指针
    robTail := Mux(robTail === 31.U, 0.U, robTail + 1.U)
  ...
  def retireHead(): Unit = {
    ...
    // 更新队头指针
    robHead := Mux(robHead === 31.U, 0.U, robHead + 1.U)
  ...
  when(io.branchFlush && !io.globalFlush) {
    // 生成一个包含每一项是否触发了 CSR 冲刷的序列 (Seq[Bool])
    val csrFlushSignals = robQueue.map { entry =>
      val shouldFlush = (entry.branchMask & io.branchOH).orR
      val isCsr = entry.specialInstr === SpecialInstr.CSR ||
      ...
      shouldFlush && isCsr && entry.busy // 只有原本忙的项被冲刷才算
    }

    // 将这些信号通过一个或门树合并成一个最终信号
    val anyCsrFlushed = csrFlushSignals.reduce(_ || _)
    ...
    // 更新队尾指针到下一个有效 Entry
    robTail := io.branchRobId + 1.U
    // 更新指令纪元
    iEpoch := iEpoch + 1.U

    // 更新对应 Entry 的 prediction 字段（使用 branchRobId 定位）
    robQueue(io.branchRobId).prediction.taken := !robQueue(
      io.branchRobId
    ).prediction.taken
  }.otherwise {
    // 预测正确，移除 branchMask
    for (entry <- robQueue) {
      when(entry.busy){
        entry.branchMask := entry.branchMask & ~io.branchOH
      }
    }
  }
