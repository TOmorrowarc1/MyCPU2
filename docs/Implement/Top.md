...
*   **逻辑简述**：
    *  **判断当前周期 PC**：
        ...
        *   否则 `CurrentPC = NextPC`。
    *  **发起取指请求**：
        *   与 LSU（内部 I-Cache）的握手协议中 `valid` 由 `IFStall` 控制（`IFStall` 高时 `valid` 低，暂停取指）。
    *  **更新 NextPC**：
        *   若取指部分握手成功（`fire` 即 `ready && valid`），则更新 `NextPC`：将 `CurrentPC` 送入 `BranchPredict` 模块获得预测结果 `Predict`（包括目标 PC 与是否跳转），并令 `NextPC` 为预测 PC。
        *   否则 `NextPC` 保持不变。   
*   **输出**：向 LSU（内部 I-Cache）发送 `{PC, PrivMode, InsEpoch, Prediction, Exception}`。

### 1.2 Decoder
*   **职责**：将指令解析到微指令；进行寄存器重命名请求与快照请求；异常检测。
*   **输入**：来自 LSU（内部 I-Cache）的 `{Instruction, PC, PrivMode, InsEpoch, Prediction, Exception}`；来自 ROB 的 `FreeRobID`， `GlobalFlush` `CSRPending` 与来自 BRU 的 `BranchFlush`。
*   **逻辑简述**：
...
*   **职责**：作为整个内存系统的统一接口，处理所有内存访问（指令取指和数据访存），维护访存顺序一致性，处理地址生成与权限检查，管理全局控制信号。
*   **输入**：
    *   **指令取指**：来自 Fetcher 的 `FetchReq`（PC、上下文信息）。
    *   **数据访存**：来自 RS 的 `{LSUOp, Imm, RobID, PrivMode, D-Epoch, BranchMask}`。
    *   **寄存器数据**：来自 PRF 的 `{BaseAddr, StoreData}`。
    *   **ROB 提交**：来自 ROB 的 `CommitStore/CommitIO` 信号。
    *   **全局控制**：来自 CSRsUnit 的 `GlobalFlush`，来自 BRU 的 `BranchFlush`，来自 ROB 的 `FENCE.I/SFENCE.VMA` 信号。
    *   **CSR 配置**：来自 CSRsUnit 的 PMP 配置。
*   **逻辑简述**：
    *   **指令取指**：接收 Fetcher 的取指请求，通过 I-Cache（内部）获取指令数据，返回给 Fetcher。
    *   **AGU**：计算 `PA = Base + Imm`（物理地址直接计算）。
    *   **权限检查**：进行 PMP 检查。若异常，标记 `Exception`。
    *   **Load 处理**：
        *   查 Store Queue (Forwarding)，若无冲突则发往 D-Cache（内部）。
        *   若为 I/O：进入 `Wait_Commit` 状态，等待 ROB 信号。
    *   **Store 处理**：将 `{PA, StoreData}` 写入 Store Queue。**不发送总线请求**，等待 ROB Commit 信号。
    *   **FENCE.I 处理**：清空 I-Cache（内部）的所有有效位，将脏行写回主存。
    *   **SFENCE.VMA 处理**：清空 I-Cache 和 D-Cache（内部）的 TLB 相关缓存。
    *   **冲刷**：响应 `GlobalFlush` (清空所有) 和 `BranchFlush` (根据 Mask 清空 Load，Store 保留直到退休)。
*   **输出**：
    *   **指令取指响应**：向 Fetcher 发送 `FetchResp`（指令数据、上下文、异常）。
    *   向 CBD 发送 `{RobID, LoadData, Exception}`。
    *   向 D-Cache（内部）发送 `{Req, D-Epoch}`。
    *   向 ROB 发送 `StoreAck`。
    *   向 ROB 发送 `FenceIDone` 信号。
...
#### 4.1.1 职责
MemorySystem 是整个内存系统的顶层模块，负责：
*   **指令取指**：通过 I-Cache 提供指令取指服务
*   **数据访存**：通过 D-Cache 提供数据读写服务
*   **总线仲裁**：通过 AXIArbiter 协调 I-Cache 和 D-Cache 的总线请求
*   **主存访问**：通过 MainMemory 提供物理内存访问
*   **Load/Store 管理**：通过 LSU 管理 Load/Store 指令的执行，包括 LSQ、地址生成与权限检查

#### 4.1.2 组成
MemorySystem 包含以下子模块：
*   **MainMemory**：物理内存模拟模块
*   **AXIArbiter**：总线仲裁器
*   **Cache**：I-Cache（指令缓存）和 D-Cache（数据缓存）
*   **LSU**：Load Store Unit，内部包含 LSQ、AGU、PMP.

### 4.2 MainMemory

#### 4.2.1 职责
模拟物理内存行为（高延迟、宽带宽），支持仿真文件的预加载。

#### 4.2.2 协议定义：Wide-AXI4 接口
...
| 信号名 | 来源 | 类型 | 描述 |
| :--- | :--- | :--- | :--- |
| `IcacheReq` | I-Cache（LSU 内部） | `Decoupled(new AXIARBundle)` | 指令取指 Miss 请求 |
| `DcacheRReq`| D-Cache（LSU 内部） | `Decoupled(new AXIARBundle)` | 数据读取 Miss 请求 |
| `DcacheWReq`| D-Cache（LSU 内部） | `Decoupled(new AXIAWBundle)` + `Decoupled(new AXIWBundle)` | 数据写回 |
| `mainMemResp`| MainMemory（LSU 内部） | `Flipped(Decoupled(new AXIRBundle))` + `Flipped(Decoupled(new AXIBBundle))` | 总线回传的数据与写确认 |

##### 4.3.3.2 输出 (Outputs)
| 信号名 | 目标 | 类型 | 描述 |
| :--- | :--- | :--- | :--- |
| `IcacheResp` | I-Cache（LSU 内部） | `Flipped(Decoupled(new AXIRBundle))` | 返回 512-bit 指令块 |
| `DcacheResp` | D-Cache（LSU 内部） | `Flipped(Decoupled(new AXIRBundle))` + `Flipped(Decoupled(new AXIBBundle))` | 返回 512-bit 数据块或写回确认 |
| `mainMemReq` | MainMemory（LSU 内部） | `WideAXI4Bundle` | 发向主存的统一 AXI 请求 |

#### 4.3.4 内部逻辑简述

##### 4.3.4.1 Ready 信号处理 (仲裁逻辑)
...
### 4.4 Cache (I-Cache 和 D-Cache)

#### 4.4.1 职责
作为 MemorySystem 内部的缓存模块，负责：
*   **物理地址缓存**：提供基于物理地址（PA）的高速数据读取与写入。
*   **主存访问**：通过 512-bit 宽总线向主存发起请求。
*   **一致性**：执行 FENCE.I 的脏行清空。
*   **冲刷**：配合 Epoch/BranchMask 处理在途指令的逻辑废除。
  
#### 4.4.2 组成（维护状态）
*   Status(Pipeline, AXIPending, FENCEIActive)
*   ReqReg(当前请求寄存器，包含 robId, PA, branchMask, epoch 等信息)
*   Tag Array (SRAM), Data Array (SRAM), Status Array (Regs)
*   AXIReqBuffer (Reg, Size=2)
*   FENCE.I Controller(记录当前扫描到行数与 Done).
  
#### 4.4.3 接口

##### 4.4.3.1 I-Cache 接口
**输入**：
*   **来自 Fetecher**：`{PC, PrivMode, InsEpoch, Prediction, Exception}`。
*   **来自 AXIArbiter**：`{Ctx(Epoch), RData}`。
*   **来自 ROB**：`fenceIReq`。

**输出**：
*   **向 Decoder**：`{Instruction, PC, PrivMode, InsEpoch, Prediction, Exception}`。
*   **向 AXIArbiter**：`{arAddr, User(Ctx)}`。
*   **向 ROB**：`fenceIDone`。

##### 4.4.3.2 D-Cache 接口
**输入**：
*   **来自 LSU**：`{PA, Width, Ctx(branchMask, robId, Epoch)}`。
*   **来自 LSU**：`{PA, Width, Ctx(branchMask, robId, Epoch), wData}`。
*   **来自 AXIArbiter**：`{Ctx(Epoch), RData, BValid}`。
*   **来自 ROB**：`currentEpoch`
*   **来自 控制平面**：`globalFlush`, `branchFlush`, `branchOH`。

**输出**：
*   **向 LSU**：`{ReadData, Ctx}` 或 `{B, Ctx}`。
*   **向 AXIArbiter**：`{arAddr, awAddr, wData, wStrb, User(Ctx)}`。

#### 4.4.4 内部逻辑简述

##### 4.4.4.1 I-Cache 逻辑
*   **Ready 反压**：只有 `AXIReq` 为空且 `!isFenceIActive` 时才从 LSU 获取 Request。
*   **内部流水线逻辑**：使用两级流水线锁存请求信息。
    *   **Stage 1**：锁存进入请求的 `PC`、`PrivMode`、`InsEpoch`、`Prediction`。发起对 Tag 和 Data SRAM 的同步读取。
    *   **Stage 2**：请求信息前进一位。由于输入已是物理地址，直接比对 `SRAMTag` 与 `ReqPATag`。
    *   **结果分发**：如果比对成功，则根据锁存的请求信息，将读结果与 Ctx 一并送入 Decoder；如果 Miss，则开始总线访问主存。
*   **缺失与回写逻辑**：
    ...
    *   请求结果返回后，清空 AXIReqBuffer，如正常执行一般返回结果，状态切回 `Pipeline`。
*   **冲刷逻辑**：
    *   **逻辑撤销**：Instruction Fetch 的逻辑撤销由 Decoder 进行，I-cache 只原样给出结果即可。
*   **FENCE.I 序列化**：
...
*   **内部流水线逻辑**：使用两级流水线锁存请求信息，有两条流水线，分别对应读与写。
    ...
    *   **Stage 2**：请求信息前进一位。由于输入已是物理地址，直接比对 `SRAMTag` 与 `ReqPATag`。
    *   **结果分发**：如果比对成功，则根据锁存的请求信息，将读结果与 Ctx 送回 LSU 或将写结果写入对应 Cache Line 并置脏位，返回写响应与 Ctx ；如果 Miss，则开始总线访问主存。
*   **缺失与回写逻辑**：
    *   若 `Miss` 且该行 `Dirty`，将状态由 `Pipeline` 切换到 `AXIReqPending`，生成的总线事务压入 AXIReqBuffer，该周期不接收新的指令进入 Cache。
    *   通过 AXIArbiter 发起总线请求，携带 `Epoch` 以便后续冲刷逻辑使用。
    *   先发起写请求，再发起读请求。
...
*   **冲刷逻辑**：
    *   **逻辑撤销**：若 `branchFlush` 命中在途指令寄存器的 `Ctx.mask`，该请求匹配不返回信息，不匹配不进行主存访问，实现方式为在在途流水线中额外添加 abort 位，若 entry abort 则不向 LSU 返回结果。
...
### 4.6 LSU (Load Store Unit)

#### 4.6.1 职责
LSU 作为 Data 部分内存系统的请求发出者，负责：
*   **数据访存管理**：管理 Load/Store 指令的发射与完成，确保乱序执行环境下的内存一致性与正确性，具体来说维护 Load Queue 和 Store Queue，处理 Store-Load 转发。
*   **地址生成与权限检查**：通过 AGU & PMP 模块计算物理地址并进行权限检查
*   **Cache 访问**：连接 D-Cache，向其发出数据读/写请求，将结果广播到 CDB 上。

#### 4.6.2 维护状态
LSU 的状态核心是 LSQ（Load Queue）和 SQ（Store Queue），每个队列都有 8 项，分别记录在途 Load 和 Store 指令的相关信息。
*   **LQ (Load Queue)**: 每个 Entry 记录如下信息：
    *   `memWidth`, `rd`, `robId`, `branchMask`, `epoch`, `privMode`, `rs1Tag`, `rs1Ready`。
    *   `PAReady` `PA` (计算并检验后返回的物理地址)。
    *   `Exceptions` (同时可能返回的异常)。
    *   `storeMask` (8 位位向量，标记依赖的 Store)。
    *   `isForwarding` (标记是否发生 Store-Load 转发)。
    *   `ForwardingData` (如果发生 Store-Load 转发，记录被转发的数据)。
*   **SQ (Store Queue)**: 每条指令记录如下信息：
...
**输入**:
*   **LSU 分发**：`lsuDispatch`（包含操作码、位宽、数据包、robId、分支掩码、特权级）
*   **Cache 响应**：`cacheRResp`（读取回复，包含数据、上下文）`cacheWResp`（写入回复，包含上下文）
*   **来自 ROB**：`storeEnable`（Store 指令退休信号），`FENCEIReq`（FENCE.I 请求信号），`currentEpoch`
*   **来自 CSR**：`csrPMP`（PMP 配置寄存器值）
*   **全局控制**：`globalFlush`、`branchFlush`、`branchOH`
*   **CDB 监听**：获取操作数就绪信号
*   **PRF 数据**：从 PRF 获取数据结果

**输出**:
*   **CDB 消息**：`cdb`（robId、phyRd、data、exception）

#### 4.6.4 内部逻辑

##### 4.6.4.1 指令接收与分发
当 `lsuDispatch.valid` 且对应队列有空位时，锁存指令信息到 LQ 或 SQ。Load 和 Store 分开处理，分别进入 LQ 和 SQ。

##### 4.6.4.2 PA 计算
*   当指令进入 LQ/SQ 时，检查 src1 是否 `Ready`，若否，监听 CDB 以期更新 `src1Ready`。
*   同理处理 src2。
*   当 `rs1Ready`时，向 AGU&PMP 发起地址生成请求。
*   当 AGU&PMP 返回 PA 和 Exception 时，锁存 PA 并标记 `PAReady`。如果有异常，记录异常信息。

##### 4.6.4.3 依赖追踪与 Store-Load 转发
*   每当 Load 计算出时，拷贝当前 SQ 中有效，PA 尚未计算或与 Load 重复的对应位作为 `storeMask`。
*   当 Store 计算出 PA 后，广播 `PA + sqIndex`，LQ 中所有 `storeMask` 对应位为 1 的 Load 立即进行地址比对。若不冲突，Load 将该位清零；若冲突，Load 保持该位。
*   当 Store 执行时，广播 `data + sqIndex`，LQ 中所有 `storeMask` 对应位为 1 的 Load 将该位清零并将 `isForwarding` 置 1，`forwardingData` 置为对应数据。
*   当 `storeMask === 0` 时，Load 指令被允许执行，若 `isForwarding` 为 1 直接提交；若为 0 则发向 D-Cache。

##### 4.6.4.4 Store 数据准备
Store 指令在发射时仅分配 SQ 条目，等待 src1 与 src2 的广播（分别为物理地址计算与存储值计算），`PA` 已计算完成且该指令位于队头后，Store 会向 CDB 发出就绪信号，而 ROB 会在其到达队头后返回信号，该 `ack` 信号到达后 store 才被允许发向 D-Cache。
...
4. **LSU → CDB**：发送 `cdb`（robId、phyRd、data、isIO、exception）
